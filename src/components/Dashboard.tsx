    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <header className="flex justify-between items-start">
        <div>
          ...
          <div className="flex items-center space-x-2 mt-1">
            <p className="text-slate-500 font-medium">
              今天还有 {pendingUnits.length} 项登记任务
            </p>
            {unsyncedCount > 0 && (
              <>
                <span className="text-slate-300 text-xs">|</span>
                <span className="text-xs font-bold text-orange-500 flex items-center">
                  <WifiOff size={10} className="mr-1" /> {unsyncedCount}{" "}
                  条记录待同步
                </span>
              </>
            )}
        ...
        <div className="bg-white p-2 rounded-2xl shadow-sm border border-white/50 flex flex-col items-center">
          <span className="text-[10px] text-[#6C5CE7] font-bold uppercase">
            Today
          </span>
      ...
      <section className="space-y-4">
        <div className="flex justify-between items-center">
          ...
          <span className="text-xs font-bold text-[#6C5CE7] bg-[#F3F0FF] px-3 py-1 rounded-full">
            进行中
          </span>
        </div>
          ...
          {pendingUnits.length > 0 ? (
            pendingUnits.map((unit, index) => (
              <div
                ...
                className={`flex-shrink-0 w-44 p-5 rounded-[32px] flex flex-col space-y-4 cursor-pointer custom-shadow transform transition-transform hover:scale-105 active:scale-95 ${
      ...
      <section className="space-y-4">
        ...
        <div className="space-y-3">
          {units.map((unit) => {
            ...
            const isWarning =
              latestLog &&
              (latestLog.temperature > unit.maxTemp ||
                latestLog.temperature < unit.minTemp);

            return (
              <div
                key={unit.id}
                className={`bg-white p-4 rounded-[28px] border border-slate-100 flex items-center justify-between transition-all ${
                  isInactive
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <div className="flex items-center text-[10px] text-slate-400 mt-0.5 space-x-2">
                      ...
                      <span className="flex items-center">
                        <MapPin size={10} className="mr-0.5" /> {unit.minTemp}~
                        {unit.maxTemp}°C
                      </span>
                    </div>
                  </div>
