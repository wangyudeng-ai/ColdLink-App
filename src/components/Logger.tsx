  ...
  return (
    <div className="max-w-xl mx-auto space-y-6 animate-in fade-in zoom-in-95 duration-500">
      <header className="flex items-center justify-between">
        <button
          onClick={onCancel}
          className="w-10 h-10 bg-white rounded-xl shadow-sm border border-slate-100 flex items-center justify-center hover:bg-slate-50"
        >
          <ChevronLeft size={20} />
        </button>
      ...
      <form
        ...
        className="bg-white rounded-[40px] p-8 shadow-xl border border-white/50 space-y-8"
      >
        <div className="space-y-4">
          <label className="text-sm font-bold text-slate-400 uppercase tracking-widest block px-1">
            选择冷库
          </label>
        ...
        <div className="space-y-4 bg-slate-50/50 p-6 rounded-[32px] border border-slate-100">
          ...
          <div className="relative flex items-center">
            ...
            />
            <span className="absolute right-4 text-3xl font-bold text-slate-300">
              °C
            </span>
          </div>
          {selectedUnit?.storedItems && (
            <div className="flex justify-center">
              <span className="text-[11px] text-slate-400 flex items-center bg-white px-3 py-1 rounded-full shadow-sm">
                <Package size={12} className="mr-1" /> 存放:{" "}
                {selectedUnit.storedItems}
              </span>
            </div>
          )}
        ...
        <button
          ...
          className={`w-full py-5 rounded-[28px] font-bold text-lg flex items-center justify-center space-x-2 transition-all transform active:scale-95 ${
            isSubmitting
              ...
              : "bg-[#6C5CE7] text-white shadow-2xl shadow-purple-200 hover:bg-[#5B4BC4]"
          }`}
        >
          {isSubmitting ? (
