...
const Analytics: React.FC<AnalyticsProps> = ({ units, logs }) => {
  ...
  const [startDateStr, setStartDateStr] = useState(format(subDays(new Date(), 6), 'yyyy-MM-dd'));
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-3xl font-extrabold text-[#1A1C1E] tracking-tight">趋势分析</h1>
          <p className="text-slate-500 font-medium">监测冷库历史温度波动的核心看板</p>
        </div>
        ...
        <div className="relative">
          ...
          >
            ...
            <div className="text-left">
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">选择日期范围</p>
              <p className="text-sm font-bold text-slate-700">{startDateStr} 至 {endDateStr}</p>
            </div>
            <ChevronDown size={18} className={`text-slate-300 transition-transform ${isRangeOpen ? 'rotate-180' : ''}`} />
          </button>
            ...
            <div className="absolute top-full right-0 mt-3 w-72 bg-white rounded-[32px] shadow-2xl border border-slate-50 p-6 z-[60] animate-in fade-in zoom-in-95 duration-200">
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  <div>
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 block px-1">开始日期</label>
                    <input
                      type="date"
                      value={startDateStr}
                  ...
                  <div>
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 block px-1">结束日期</label>
                    <input
                      type="date"
                      value={endDateStr}
          ...
          {chartData.length > 0 ? (
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <Tooltip 
                  ...
                  itemStyle={{ fontWeight: 'bold', fontSize: '14px', color: '#6C5CE7' }}
                  labelStyle={{ fontWeight: 'bold', color: '#94a3b8', fontSize: '10px', marginBottom: '4px' }}
                />
                {selectedUnit && (
                  <>
                    <ReferenceLine y={selectedUnit.maxTemp} label={{ position: 'right', value: 'MAX', fill: '#FF823C', fontSize: 8, fontWeight: 'bold' }} stroke="#FF823C" strokeDasharray="5 5" opacity={0.5} />
                    <ReferenceLine y={selectedUnit.minTemp} label={{ position: 'right', value: 'MIN', fill: '#3CA7FF', fontSize: 8, fontWeight: 'bold' }} stroke="#3CA7FF" strokeDasharray="5 5" opacity={0.5} />
                  </>
                )}
                <Line 
                  type="monotone" 
                  ...
                  strokeWidth={4} 
                  dot={{ r: 0 }}
                  activeDot={{ r: 6, strokeWidth: 4, stroke: '#fff', fill: '#6C5CE7' }}
            ...
            <div className="h-full flex flex-col items-center justify-center text-slate-300 space-y-4">
              ...
              <div className="text-center">
                ...
                <p className="text-[10px] uppercase tracking-widest mt-1">请尝试调整筛选范围</p>
              </div>
            </div>
          )}
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: '平均温度', val: chartData.length > 0 ? (chartData.reduce((acc, curr) => acc + curr.temp, 0) / chartData.length).toFixed(1) + '°C' : '--', color: 'text-slate-700' },
          { label: '最高温度', val: chartData.length > 0 ? Math.max(...chartData.map(d => d.temp)) + '°C' : '--', color: 'text-[#FF823C]' },
          { label: '最低温度', val: chartData.length > 0 ? Math.min(...chartData.map(d => d.temp)) + '°C' : '--', color: 'text-[#3CA7FF]' },
          { label: '记录点数', val: chartData.length + ' 次', color: 'text-[#6C5CE7]' },
        ].map((stat, i) => (
